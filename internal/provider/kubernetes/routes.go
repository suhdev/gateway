...
const (
	// routeListBatchSize defines the batch size for paginated route listing.
	// This helps prevent memory issues and improves performance when processing large numbers of routes.
	routeListBatchSize = 1000
)
...
func (r *gatewayAPIReconciler) processTLSRoutes(ctx context.Context, gatewayNamespaceName string,
	...
	// Process TLSRoutes in batches using pagination
	continueToken := ""
	totalProcessed := 0
	...
	for {
		...
		listOpts := &client.ListOptions{
			...
			Limit:         routeListBatchSize,
			Continue:      continueToken,
		}

		if err := r.client.List(ctx, tlsRouteList, listOpts); err != nil {
			r.log.Error(err, "unable to find associated TLSRoutes")
			return err
		}

		batchSize := len(tlsRouteList.Items)
		if batchSize > 0 {
			r.log.V(1).Info("processing TLSRoute batch",
				"batchSize", batchSize,
				"totalProcessed", totalProcessed,
				"gateway", gatewayNamespaceName)
		}
		...
		continueToken = tlsRouteList.GetContinue()
		if continueToken == "" {
			break
		}
	}
	...
	if totalProcessed > 0 {
		r.log.Info("completed processing TLSRoutes",
...
func (r *gatewayAPIReconciler) processGRPCRoutes(ctx context.Context, gatewayNamespaceName string,
	...
	// Process GRPCRoutes in batches using pagination
	continueToken := ""
	totalProcessed := 0
	...
	for {
		...
		if err := r.client.List(ctx, grpcRouteList, listOpts); err != nil {
			r.log.Error(err, "failed to list GRPCRoutes")
			return err
		}

		batchSize := len(grpcRouteList.Items)
		if batchSize > 0 {
			r.log.V(1).Info("processing GRPCRoute batch",
				"batchSize", batchSize,
				"totalProcessed", totalProcessed,
				"gateway", gatewayNamespaceName)
		}
		...
		// Check if there are more results to fetch
		continueToken = grpcRouteList.GetContinue()
		if continueToken == "" {
			break
		}
	}
	...
	if totalProcessed > 0 {
		r.log.Info("completed processing GRPCRoutes",
			"total", totalProcessed,
			"gateway", gatewayNamespaceName)
	}
...
func (r *gatewayAPIReconciler) processHTTPRoutes(ctx context.Context, gatewayNamespaceName string,
	...
	// Process HTTPRoutes in batches using pagination to handle large numbers of routes efficiently
	continueToken := ""
	totalProcessed := 0
	...
	for {
		...
		if err := r.client.List(ctx, httpRouteList, listOpts); err != nil {
			r.log.Error(err, "failed to list HTTPRoutes")
			return err
		}

		batchSize := len(httpRouteList.Items)
		if batchSize > 0 {
			r.log.V(1).Info("processing HTTPRoute batch",
				"batchSize", batchSize,
				"totalProcessed", totalProcessed,
				"gateway", gatewayNamespaceName)
		}
		...
		// Check if there are more results to fetch
		continueToken = httpRouteList.GetContinue()
		if continueToken == "" {
	...
	if totalProcessed > 0 {
		r.log.Info("completed processing HTTPRoutes",
			"total", totalProcessed,
			"gateway", gatewayNamespaceName)
	}
...
func (r *gatewayAPIReconciler) processTCPRoutes(ctx context.Context, gatewayNamespaceName string,
	...
	// Process TCPRoutes in batches using pagination
	continueToken := ""
	totalProcessed := 0
	...
	for {
		...
		if err := r.client.List(ctx, tcpRouteList, listOpts); err != nil {
			r.log.Error(err, "unable to find associated TCPRoutes")
			return err
		}

		batchSize := len(tcpRouteList.Items)
		if batchSize > 0 {
			r.log.V(1).Info("processing TCPRoute batch",
				"batchSize", batchSize,
				"totalProcessed", totalProcessed,
				"gateway", gatewayNamespaceName)
		}
		...
		// Check if there are more results to fetch
		continueToken = tcpRouteList.GetContinue()
		if continueToken == "" {
			break
		}
	}
	...
	if totalProcessed > 0 {
		r.log.Info("completed processing TCPRoutes",
			"total", totalProcessed,
			"gateway", gatewayNamespaceName)
	}
...
func (r *gatewayAPIReconciler) processUDPRoutes(ctx context.Context, gatewayNamespaceName string,
	...
	// Process UDPRoutes in batches using pagination
	continueToken := ""
	totalProcessed := 0
	...
	for {
		...
		if err := r.client.List(ctx, udpRouteList, listOpts); err != nil {
			r.log.Error(err, "unable to find associated UDPRoutes")
			return err
		}

		batchSize := len(udpRouteList.Items)
		if batchSize > 0 {
			r.log.V(1).Info("processing UDPRoute batch",
				"batchSize", batchSize,
				"totalProcessed", totalProcessed,
				"gateway", gatewayNamespaceName)
		}
		...
		// Check if there are more results to fetch
		continueToken = udpRouteList.GetContinue()
		if continueToken == "" {
			break
		}
	}
	...
	if totalProcessed > 0 {
		r.log.Info("completed processing UDPRoutes",
			"total", totalProcessed,
			"gateway", gatewayNamespaceName)
	}
